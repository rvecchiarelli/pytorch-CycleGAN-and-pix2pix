...
def save_images_hdf5(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    """Save images to the disk.
    ...
    for label, im_data in visuals.items():
        ...
        links.append(image_name)
        if use_wandb:
            ims_dict[label] = wandb.Table(columns =['real_A', 'real_B', 'fake_B'], data=im)
    webpage.add_images(ims, txts, links, width=width)
    if use_wandb:
        wandb.log(ims_dict)
